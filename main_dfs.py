...
class game:
	def __init__(self, master = None):
		...
		self.rectangle = self.canvas.create_oval(self.rectanglexa, self.rectangleya, self.rectanglexb, self.rectangleyb, fill = "Yellow")
		self.canvas.pack()
		self.dfsPath = []
		self.dfsVisited = []
		if input("Infinite or Finite? (i/f)") == "f":
			self.dfsDirection = dfsr(currentMap, pacmanStart, pacmanGoal , self.dfsPath , self.dfsVisited, infiniteLoop=True)
		else:
			self.dfsDirection = dfsr(currentMap, pacmanStart, pacmanGoal , self.dfsPath , self.dfsVisited, infiniteLoop=False)
		print(self.dfsDirection)
		self.dfsPath.reverse()
		self.dfsVisited.reverse()
		print(self.dfsPath)
		print(self.dfsVisited)
	...
	def movement(self):
		...
		if self.xdest == 0 and self.ydest == 0:
			...
			if self.dfsVisited.__len__() != 0:
				lastNode = self.gotoNode
				self.gotoNode = self.dfsVisited.pop(0)
...
if __name__ == "__main__":
	...
	printMap(currentMap, pacmanStart, pacmanGoal)
	master = Tk()
	gfg = game(master)
