...
class game:
    def __init__(self, master=None):
        ...
        self.gotoNode = (0, 0)
        self.firstTime = True
        self.width = currentMap[0].__len__(
        ) * widthPath + (currentMap[0].__len__() + 1) * widthWall
        self.height = currentMap.__len__() * heightPath + \
            (currentMap.__len__() + 1) * heightWall
        ...
        self.goalxb = (pacmanGoal[1]) * (widthPath +
                                         widthWall) + widthWall + heightWall
        self.goalyb = (pacmanGoal[0]) * (heightPath +
                                         heightWall) + widthWall + heightWall

        self.goal = self.canvas.create_oval(
            self.goalxa + 5, self.goalya + 5, self.goalxb - 5, self.goalyb - 5, fill="Green")
        self.rectanglexa = pacmanStart[1] * (widthPath + widthWall) + widthWall
        self.rectangleya = pacmanStart[0] * \
            (heightPath + heightWall) + heightWall
        self.rectanglexb = (
            pacmanStart[1]) * (widthPath + widthWall) + widthWall + heightWall
        self.rectangleyb = (
            pacmanStart[0]) * (heightPath + heightWall) + widthWall + heightWall

        self.rectangle = self.canvas.create_oval(
            self.rectanglexa, self.rectangleya, self.rectanglexb, self.rectangleyb, fill="Yellow")
        self.canvas.pack()
        self.bfsPath = []
        self.bfsVisited = []
        self.bfsVisited, self.bfsPath = bfs(
            currentMap, pacmanStart, pacmanGoal)
        ...
        for i in range(len(currentMap)):
            for j in range(len(currentMap[i])):
                if currentMap[i][j] == 1:
                    ...
                    yb = (i) * (heightPath + heightWall) + \
                        widthWall + heightWall
                    self.canvas.create_rectangle(
                        xa, ya, xb, yb, fill="blue", width=widthPath)
    ...
    def movement(self):
        ...
        if self.xdest == 0 and self.ydest == 0:
            ...
            if self.dfsPath.__len__() != 0:
                ...
                self.xdest = (
                    self.gotoNode[1] - lastNode[1]) * (widthPath + widthWall)
                print(self.xdest)
                ...
                self.ydest = (
                    self.gotoNode[0] - lastNode[0]) * (heightPath + heightWall)
...
if __name__ == "__main__":
    ...
    if input() == "y":
        while True:
            ...
            pacmanStart = (int(input("Start Row : ")),
                           int(input("Start Col : ")))
